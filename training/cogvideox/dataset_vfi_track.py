...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors:
            ...
            first_image, video, last_image = self._preprocess_video(self.video_paths[index])
            pred_track, pred_visibility = torch.load(self.track_paths[index]).to('cpu'), torch.load(self.visibility_paths[index]).to('cpu')
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str]]: 
        ...
        track_path = self.data_root.joinpath(self.track_column)
        visibility_path = self.data_root.joinpath(self.visibility_column)
        ...
        if not track_path.exists() or not track_path.is_file():
            raise ValueError(
                "Expected `--track_path` to be path to a file in `--data_root` containing line-separated .pt."
            )
        if not visibility_path.exists() or not visibility_path.is_file():
            raise ValueError(
                "Expected `--visibility_path` to be path to a file in `--data_root` containing line-separated .pt."
            )
        ...
        with open(track_path, "r", encoding="utf-8") as file:
            track_paths = [self.data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        with open(visibility_path, "r", encoding="utf-8") as file:
            visibility_paths = [self.data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        ...
        return prompts, video_paths, track_paths, visibility_paths
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def _preprocess_video(self, path: Path) -> torch.Tensor:
        if self.load_tensors:
            ...
            first_image = frames[:1].clone() if self.image_to_video else None
            last_image = frames[-1:].clone() if self.image_to_video else None

            return first_image, frames, last_image
