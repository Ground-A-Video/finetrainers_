...
#
from pipeline_cogvideox_image2video_interpolation_track import CogVideoXInterpolationTrackPipeline
#
...
from dataset_vfi_track import BucketSampler, VideoDatasetWithResizing, VideoDatasetWithResizeAndRectangleCrop  # isort:skip
...
def warp_video_with_tracks(video, pred_tracks, pred_visibility):
...
def log_validation(
    ...
    pipe: CogVideoXInterpolationTrackPipeline,
    args: Dict[str, Any],
...
def run_validation(
    ...
    for validation_videos_dir_i in validation_videos_dir:
        ...
        pred_tracks = torch.load(os.path.join(validation_videos_dir_i, "pred_tracks.pt")).to(accelerator.device)
        track_downsample_step = args.track_downsample_step if args.track_downsample_step is not None else 8
        pred_tracks = pred_tracks / track_downsample_step
        pred_visibility = torch.load(os.path.join(validation_videos_dir_i, "pred_visibility.pt")).to(accelerator.device)
...
class CollateFunction:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        pred_tracks = [x["pred_track"] for x in data[0]][0]

        pred_visibilitys = [x["pred_visibility"] for x in data[0]][0]
        ...
        return {
            ...
            "prompts": prompts,
            "pred_tracks": pred_tracks,
            "pred_visibilitys": pred_visibilitys
        }
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            CogVideoXInterpolationTrackPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = CogVideoXInterpolationTrackPipeline.lora_state_dict(input_dir)
    ...
    #### NOTE: ALSO FINETUNE 'transformer.patch_embed.proj' ####
    transformer_proj_params = list(transformer.patch_embed.proj.parameters())
    for p in transformer_proj_params:
        p.requires_grad = True
    ############################################################

    # Optimization parameters
    transformer_parameters_with_lr = {
        "params": transformer_lora_parameters + transformer_proj_params,
    ...
    dataset_init_kwargs = {
        ...
        "visibility_column": args.visibility_column,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                pred_tracks = batch["pred_tracks"].to(accelerator.device, non_blocking=True)
                pred_visibilitys = batch["pred_visibilitys"].to(accelerator.device, non_blocking=True)
                ...
                warped_video = warp_video_with_tracks(image_latents.detach().clone(), pred_tracks, pred_visibilitys)     # [B, F, C, H, W]
